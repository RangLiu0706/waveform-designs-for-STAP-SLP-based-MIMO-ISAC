% The proposed MM-neADMM algorithm for PAPR-constrained waveform design.
...
subject to
t <= real(H*x);
abs(x) <= sqrt(ratio*P/(M*N*Nt));
square_pos(norm(x,2)) <= P;
cvx_end
...
while iter < Nmax && res > res_th
    ...
    %%% update y
    tt = x + lambda/rho;
    y = tt/norm(tt,2)*sqrt(P);
    %%% update lambda
    lambda = lambda + rho*(x-y);

    iter = iter + 1;
    vres(iter) = norm(x-y,2)^2;
    vobj(iter) = real(-x'*bt) + norm(x-y+lambda/rho,2)^2*rho/2;
...
vobj(mi+1:end) = vobj(mi)*ones(1,Nmax-mi);
vres(mi+1:end) = vres(mi)*ones(1,Nmax-mi);
SINRiter(mi+1:end) = ms*ones(1,Nmax-mi);

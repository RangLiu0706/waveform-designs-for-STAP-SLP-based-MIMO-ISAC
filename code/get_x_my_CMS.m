% The proposed MM-neADMM algorithm for constant-modulus with similarity constraint waveform design.
...
for i = 1:1:Nt
    for j = 1:1:M*N
        X0(i,j) = sqrt(P/M/N/Nt)*exp(1i*2*pi*i*(j-1)/M/N)*exp(1i*pi*(j-1)^2/M/N);
    end
end
x0 = vec(X0);

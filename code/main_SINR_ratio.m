...
ratio_range = (1:0.2:2);
N_sim = 100;
SINR_my_PAR1 = zeros(1,length(ratio_range));
SINR_my_PAR2 = zeros(1,length(ratio_range));
SINR_my_PAR3 = zeros(1,length(ratio_range));

Prms.Ku = Ku; Prms.Nt = Nt; Prms.Nr = Nr; Prms.M = M; Prms.N = N;
Prms.Phi = Phi; Prms.Nmax = Nmax; Prms.res_th = res_th; Prms.sigma2 = sigma2;
Prms.sigma_t2 = sigma_t2; Prms.A = A; Prms.beta = sin(Phi)*sqrt(sigman2*10^(0.1*SNR));
...
for sim = 1:N_sim
    ...
    for ratio_index = 1:1:length(ratio_range)
        ratio = ratio_range(ratio_index)
        Prms.ratio = ratio;

        Prms.P = 30;
        [x_my_par1,SINRiter_my_par1] = get_x_my_PAPR(Prms,H,S,1);
        SINR_my_PAR1(ratio_index) = SINR_my_PAR1(ratio_index) + max(SINRiter_my_par1);

        Prms.P = 40;
        [x_my_par2,SINRiter_my_par2] = get_x_my_PAPR(Prms,H,S,1);
        SINR_my_PAR3(ratio_index) = SINR_my_PAR3(ratio_index) + max(SINRiter_my_par2);

        Prms.P = 50;
        [x_my_par3,SINRiter_my_par3] = get_x_my_PAPR(Prms,H,S,1);
        SINR_my_PAR2(ratio_index) = SINR_my_PAR2(ratio_index) + max(SINRiter_my_par3);

    end
    toc
end

SINR_my_PAR1 = SINR_my_PAR1/sim;
SINR_my_PAR3 = SINR_my_PAR3/sim;
SINR_my_PAR2 = SINR_my_PAR2/sim;

figure;
plot(ratio_range-1,SINR_my_PAR1,'-s','color',[0,0.5,0],'LineWidth',1.5)
hold on
plot(ratio_range-1,SINR_my_PAR3,'-o','color',[0.7,0,0],'LineWidth',1.5)
plot(ratio_range-1,SINR_my_PAR2,'-^','color',[0,0,0.7],'LineWidth',1.5)
hold off
xlabel('PAPR {\it \epsilon}');
ylabel('Radar SINR (dB)');
grid on
legend('Proposed, PAPR, {\it P} = 30W','Proposed, PAPR, {\it P} = 40W','Proposed, PAPR, {\it P} = 50W');
axis([0 1 23 32.5])

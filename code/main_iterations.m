...
%%% clutter
Nc = 100; %%% the numebr of clutter patches in each range cell
...
res_th = 0;   %%%% convergence tolerance
SNR = 5;      %%% SNR

N_sim = 100;
VSINR_my_CE = zeros(1,Nmax);
VSINR_my_CES = zeros(1,Nmax);
Vres_my_PAR = zeros(1,Nmax);
VSINR_my_PAR = zeros(1,Nmax);
...
for sim = 1:N_sim
    ...
    VSINR_my_CE = VSINR_my_CE + SINRiter_my_ce;

    [x_my_ces,SINRiter_my_ces] = get_x_my_CMS(Prms,H,S,rho*5);
    VSINR_my_CES = VSINR_my_CES + SINRiter_my_ces;

    [x_my_par,SINRiter_my_par] = get_x_my_PAPR(Prms,H,S,1);
    VSINR_my_PAR = VSINR_my_PAR + SINRiter_my_par;

    toc
end

VSINR_my_CE = VSINR_my_CE/sim;
VSINR_my_CES = VSINR_my_CES/sim;
VSINR_my_PAR = VSINR_my_PAR/sim;

figure;
plot(VSINR_my_CE,'-','color',[0.7,0,0],'LineWidth',1.5)
hold on
plot(VSINR_my_CES,'-','color',[0,0.5,0],'LineWidth',1.5)
plot(VSINR_my_PAR,'-','color',[0,0,0.7],'LineWidth',1.5)
hold off
xlabel('Number of iterations');
ylabel('Radar SINR (dB)');
grid on
legend('Proposed, CM','Proposed, CMS','Proposed, PAPR')
axis([1 100 min(VSINR_my_CES)-0.2 max(VSINR_my_PAR)+0.2])

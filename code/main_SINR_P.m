...
P_range = (25:5:50);
N_sim = 100;
SINR_my_CE = zeros(1,length(P_range));
SINR_my_PAR = zeros(1,length(P_range));
SINR_my_CES = zeros(1,length(P_range));

ratio = 2; %%% PAPR level
Prms.Ku = Ku; Prms.Nt = Nt; Prms.Nr = Nr; Prms.M = M; Prms.N = N;
Prms.Phi = Phi; Prms.Nmax = Nmax; Prms.res_th = res_th; Prms.sigma2 = sigma2;
Prms.sigma_t2 = sigma_t2; Prms.A = A; Prms.ratio = ratio;
...
for sim = 1:N_sim
    ...
    for P_index = 1:1:length(P_range)
        ...
        Prms.P = Pt; Prms.epsi = epsi;

        [x_my_ce,SINRiter_my_ce] = get_x_my_CM(Prms,H,S,rho);
        SINR_my_CE(P_index) = SINR_my_CE(P_index) + max(SINRiter_my_ce);

        [x_my_ces,SINRiter_my_ces] = get_x_my_CMS(Prms,H,S,rho*5);
        SINR_my_CES(P_index) = SINR_my_CES(P_index) + max(SINRiter_my_ces);

        [x_my_par,SINRiter_my_par] = get_x_my_PAPR(Prms,H,S,1);
        SINR_my_PAR(P_index) = SINR_my_PAR(P_index) + max(SINRiter_my_par);
...
figure;
plot(P_range,SINR_my_CE,'-o','color',[0.7,0,0],'LineWidth',1.5)
hold on;
plot(P_range,SINR_my_CES,'-s','color',[0,0.5,0],'LineWidth',1.5)
plot(P_range,SINR_my_PAR,'-^','color',[0,0,0.7],'LineWidth',1.5)
hold off
xlabel('Total transmit power {\it P} (W)');

...
SNR_dB_range = (-10:4:10);
N_sim = 100;
SINR_my_CE = zeros(1,length(SNR_dB_range));
SINR_my_PAR = zeros(1,length(SNR_dB_range));
SINR_my_CES = zeros(1,length(SNR_dB_range));
...
for sim = 1:N_sim
    ...
    for SNR_index = 1:1:length(SNR_dB_range)
        ...
        SINR_my_CE(SNR_index) = SINR_my_CE(SNR_index) + max(SINRiter_my_ce);

        [x_my_ces,SINRiter_my_ces] = get_x_my_CMS(Prms,H,S,rho*5);
        SINR_my_CES(SNR_index) = SINR_my_CES(SNR_index) + max(SINRiter_my_ces);

        [x_my_par,SINRiter_my_par] = get_x_my_PAPR(Prms,H,S,1);
        SINR_my_PAR(SNR_index) = SINR_my_PAR(SNR_index) + max(SINRiter_my_par);
...
figure;
plot(SNR_dB_range,SINR_my_CE,'-o','color',[0.7,0,0],'LineWidth',1.5)
hold on
plot(SNR_dB_range,SINR_my_CES,'-s','color',[0,0.5,0],'LineWidth',1.5)
plot(SNR_dB_range,SINR_my_PAR,'-^','color',[0,0,0.7],'LineWidth',1.5)
hold off
